...
public abstract class McwRecipes extends RecipeProvider
{
    ...
    protected void recipeShapelessWith1ItemRecycle(Consumer<FinishedRecipe> output, ItemLike planks, ItemLike result, int count, ItemLike firstItem, int required, String group, String originalMod, String mcwMod)
    {
        ConditionalRecipe.builder()
                ...
                .addRecipe(shapeless(RecipeCategory.BUILDING_BLOCKS, result, count)
                        ...
                        .group(group)
                        .unlockedBy(getHasName(planks), has(planks))::save).generateAdvancement()
                .build(output, ResourceLocation.fromNamespaceAndPath(Finder.getModidOfItem(result.asItem()), Finder.getIdOfItem(result.asItem()) + "_recycle"));
    }
    ...
    protected void recipeStonecutter(Consumer<FinishedRecipe> output, ItemLike result, ItemLike firstItem, String originalMod, String mcwMod)
    {
        ...
        ConditionalRecipe.builder()
                .addCondition(new ModLoadedCondition(mcwMod)).addCondition(new ModLoadedCondition(originalMod))
                .addRecipe(c -> SingleItemRecipeBuilder.stonecutting(Ingredient.of(firstItem), RecipeCategory.BUILDING_BLOCKS, result)
                        .unlockedBy(getHasName(firstItem), has(firstItem)).save(c, recipeId)).generateAdvancement()
