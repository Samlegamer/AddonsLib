        ...
        for(ModType mod : type)
        {
            for(Map.Entry<String, BlockSoundGroup> map : orderedMap.entrySet())
            {
                ...
                for(BlockId blockId : mcwBlockIdBase.blocks())
                {
                    ...
                    AbstractBlock.Settings prop = McwProperties.getWoodProperties(mat, mod).sounds(soundType).registryKey(RegistryKey.of(RegistryKeys.BLOCK, Identifier.of(MODID, id)));
    ...
    public static void setRegistriesLeave(String MODID, Map<String, BlockSoundGroup> stringSoundTypeMap)
    {
        ...
        for(BlockId blockId : mcwBlockIdBase.blocks())
        {
            for(Map.Entry<String, BlockSoundGroup> map : orderedMap.entrySet())
            {
                ...
                String id = McwBlocksIdBase.replacement(blockId.id(), mat);
                AbstractBlock.Settings prop = McwProperties.getLeaveProperties().sounds(soundType).registryKey(RegistryKey.of(RegistryKeys.BLOCK, Identifier.of(MODID, id)));
                boolean isModLoaded = modList.isModLoaded(mcwBlockIdBase.modid());
        ...
        for(ModType mod : type)
        {
            for(Map.Entry<String, BlockSoundGroup> map : orderedMap.entrySet())
            {
                ...
                for(BlockId blockId : mcwBlockIdBase.blocks())
                {
                    String id = McwBlocksIdBase.replacement(blockId.id(), mat);
                    String reflectedLocation = blockId.reflectedLocation().getFabric();
                    AbstractBlock.Settings prop = McwProperties.getStoneProperties(mod).sounds(soundType).registryKey(RegistryKey.of(RegistryKeys.BLOCK, Identifier.of(MODID, id)));
                    boolean isModLoaded = modList.isModLoaded(mcwBlockIdBase.modid());
