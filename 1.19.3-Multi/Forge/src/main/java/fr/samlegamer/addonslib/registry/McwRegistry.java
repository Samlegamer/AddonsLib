                ...
                for(BlockId blockId : mcwBlockIdBase.blocks())
                {
                    ...
                    if(isModLoaded)
                    {
                        if(isBlockDefaultState("1.17.1", reflectedLocation)) {
                            ...
                            CreateBlockReferences.createBlock(id, () -> Registration.getBlocksField(blockId.reflectedLocation().getForge(), prop, SoundEvents.WOODEN_DOOR_CLOSE, SoundEvents.WOODEN_DOOR_OPEN), block, item);
                        }
                        else {
                    ...
                    if(isModLoaded)
                    {
                        if(isBlockDefaultState("1.17.1", reflectedLocation)) {
                            ...
                            if (isFence(reflectedLocation)) {
                                ...
                                CreateBlockReferences.createBlockStone(id, () -> new FenceGateBlock(prop, SoundEvents.FENCE_GATE_CLOSE, SoundEvents.FENCE_GATE_OPEN), block, item);
                            }  else {
