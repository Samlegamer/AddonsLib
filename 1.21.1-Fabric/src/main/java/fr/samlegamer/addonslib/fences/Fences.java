			...
			{				
				try {
				    ...
				    else
				    {
				        ...
				        registryEntry(MODID, i + "_highley_gate", new FenceGateBlock(WoodType.OAK, WOOD), tab, modLoaded);
				        registryEntry(MODID, i + "_pyramid_gate", new FenceGateBlock(WoodType.OAK, WOOD), tab, modLoaded);
				    }
				} catch (Exception e) {
				    e.printStackTrace();
				}
			...
			for(String i : rock)
			{
				try {
					...
					else
		            {
						...
						registryEntry(MODID, i+"_railing_gate", new FenceGateBlock(WoodType.OAK, STONE), tab, modLoaded);
