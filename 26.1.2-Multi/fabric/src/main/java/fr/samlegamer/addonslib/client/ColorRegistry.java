		...
		for(ObjectColor objectColor : objectColors) {
			...
			if(ColorUtils.isValidBlock(block)) {
				if (color == ObjectColor.DEFAULT_COLOR) {
					BlockColorRegistry.register((state, view, pos, tintIndex) -> view != null && pos != null ? BiomeColors.getAverageFoliageColor(view, pos) : FoliageColor.get(0.5D, 1.0D), block);
				} else {
					BlockColorRegistry.register((state, view, pos, tintIndex) -> color, block);
				}
			}
		}
	}
