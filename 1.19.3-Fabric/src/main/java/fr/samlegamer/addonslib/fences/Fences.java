		...
		for (String i : set) {
			for (BlockId blockId : McwBlocksIdBase.FENCES_WOOD_BLOCKS.blocks()) {
				...
				if (blockId.reflectedLocation().contains("FenceBlock")) {
					...
					RegistryEntryReferences.registryEntry(MODID, id, new FenceGateBlock(WOOD, SoundEvents.BLOCK_FENCE_GATE_CLOSE, SoundEvents.BLOCK_FENCE_GATE_OPEN));
		...
		for (String i : rock) {
			for (BlockId blockId : McwBlocksIdBase.FENCES_STONE_BLOCKS.blocks()) {
				...
				if (blockId.reflectedLocation().contains("FenceBlock")) {
					...
					RegistryEntryReferences.registryEntry(MODID, id, new FenceGateBlock(STONE, SoundEvents.BLOCK_FENCE_GATE_CLOSE, SoundEvents.BLOCK_FENCE_GATE_OPEN));
				} else if(isModMcwLoaded) {
