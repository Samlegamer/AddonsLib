...
public class McwRecipes extends RecipeProvider
{
    ...
    private AbstractType getFromModType(ModType modType, String modid, String originalMod)
    {
        return switch (modType) {
            case BRIDGES -> new Bridges(packOutput, registries, modid, originalMod);
            case DOORS -> new Doors(packOutput, registries, modid, originalMod);
            case FENCES -> new Fences(packOutput, registries, modid, originalMod);
            case FURNITURES -> new Furnitures(packOutput, registries, modid, originalMod);
            case PATHS -> new Paths(packOutput, registries, modid, originalMod);
            case ROOFS -> new Roofs(packOutput, registries, modid, originalMod);
            case STAIRS -> new Stairs(packOutput, registries, modid, originalMod);
            case TRAPDOORS -> new Trapdoors(packOutput, registries, modid, originalMod);
            case WINDOWS -> new Windows(packOutput, registries, modid, originalMod);
            default -> null;
        };
    }
    ...
    protected void mkScW1Item(RecipeOutput output, ItemLike result, ItemLike firstItem)
    {
        ...
        ConditionalRecipe.builder()
                ...
                .recipe(SingleItemRecipeBuilder.stonecutting(Ingredient.of(firstItem), RecipeCategory.BUILDING_BLOCKS, result)
                        .unlockedBy(getHasName(firstItem), has(firstItem))::save)
                .save(output, ResourceLocation.parse(recipeId));
    }
    ...
    protected void mkRpW1Item(RecipeOutput output, ItemLike planks, String[] pattern, ItemLike result, int count, ItemLike firstItem, String group, String suffix)
    {
        ConditionalRecipe.builder()
                ...
                .group(group)
                .unlockedBy(getHasName(planks), has(planks))::save)
                .save(output, ResourceLocation.fromNamespaceAndPath(this.modid, BuiltInRegistries.ITEM.getKey(result.asItem()).getPath() + suffix));
    }
    ...
    protected void mkRpShapelessW1Item(RecipeOutput output, ItemLike planks, ItemLike result, int count, ItemLike firstItem, int required, String group, String suffix)
    {
        ConditionalRecipe.builder()
                .condition(new AndCondition(List.of(new ModLoadedCondition(this.mcwModid), new ModLoadedCondition(this.originalMod))))
                .recipe(shapeless(RecipeCategory.BUILDING_BLOCKS, result, count)
