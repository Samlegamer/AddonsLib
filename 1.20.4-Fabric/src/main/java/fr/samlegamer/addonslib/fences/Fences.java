		...
		for (String i : set) {
			for (BlockId blockId : McwBlocksIdBase.FENCES_WOOD_BLOCKS.blocks()) {
				...
				if (blockId.reflectedLocation().contains("FenceBlock")) {
					...
					RegistryEntryReferences.registryEntry(MODID, id, new FenceGateBlock(WoodType.OAK, WOOD));
		...
		for (String i : rock) {
			for (BlockId blockId : McwBlocksIdBase.FENCES_STONE_BLOCKS.blocks()) {
				...
				if (blockId.reflectedLocation().contains("FenceBlock")) {
					...
					RegistryEntryReferences.registryEntry(MODID, id, new FenceGateBlock(WoodType.OAK, STONE));
				} else if(isModMcwLoaded) {
