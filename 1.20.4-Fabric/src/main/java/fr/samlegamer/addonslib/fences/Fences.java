        ...
        for (String i : set) {
			try {
				if (AddonsLib.isLoaded(modid)) {
					...
					final Block highley_gate = new FenceGateBlock(WoodType.OAK, prop);
					final Block pyramid_gate = new FenceGateBlock(WoodType.OAK, prop);
					...
					registryEntry(MODID, i + "_highley_gate", new FenceGateBlock(WoodType.OAK, prop));
					registryEntry(MODID, i + "_pyramid_gate", new FenceGateBlock(WoodType.OAK, prop));
