...
class Doors extends AbstractType {
    ...
    public Doors(FabricDataOutput output, CompletableFuture<RegistryWrapper.WrapperLookup> registriesFuture, String modid, String originalMod)
    {
        super(output, registriesFuture, modid, originalMod, id);
    }

    private void door_variant(RecipeGenerator generator, RecipeExporter exporter, Block door_var, Block planks, Item print)
    {
        mcwRecipes.mkRpW2Items(generator, exporter, planks, new String[]{"A", "B", "A"}, door_var, 1, planks, print, Objects.requireNonNull(print.toString()).replace("print_", ""));
    }

    private void other_door(RecipeGenerator generator, RecipeExporter exporter, String mat, Block log, Block planks, Block slab)
    {
        ...
        mcwRecipes.mkRpW2Items(generator, exporter, planks, new String[]{"BB", "BB", "AA"}, japanese_door, 3, planks, Items.PAPER, "shoji");
        mcwRecipes.mkRpW2Items(generator, exporter, log, new String[]{"AB", "BB", "AB"}, bark_glass_door, 3, log, Blocks.GLASS, "bark");
        mcwRecipes.mkRpW2Items(generator, exporter, planks, new String[]{"AB", "BB", "AB"}, glass_door, 3, planks, Blocks.GLASS_PANE, "glassed");
        mcwRecipes.mkRpW2Items(generator, exporter, planks, new String[]{"BB", "AA", "AA"}, barn_door, 2, planks, Items.STICK, "barn");
        mcwRecipes.mkRpW2Items(generator, exporter, planks, new String[]{"BB", "AA", "AA"}, barn_glass_door, 3, planks, Blocks.GLASS_PANE, "barn_glass");
    }
    ...
    public void buildWood(RecipeGenerator generator, RecipeExporter exporter, List<String> MAT, List<McwWoodMat> woodMat)
    {
        ...
        if(MAT.size() == woodMat.size()) {
            for (int i = 0; i < MAT.size(); i++) {
                ...
                other_door(generator, exporter, MAT.get(i), log, planks, slab);
                ...
                for(Map.Entry<String, String> variant : variants.entrySet())
                {
                    ...
                    door_variant(generator, exporter, door_var, planks, print);
                }
