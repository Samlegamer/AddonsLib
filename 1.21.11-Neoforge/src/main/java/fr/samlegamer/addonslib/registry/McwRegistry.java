                ...
                for(BlockId blockId : mcwBlockIdBase.blocks())
                {
                    ...
                    if(isModLoaded)
                    {
                        if(isBlockDefaultState("1.17.1", reflectedLocation)) {
                            CreateBlockReferences.createBlock(id, registryName -> Registration.getBlocksField(blockId.reflectedLocation().getForge(), prop.setId(ResourceKey.create(Registries.BLOCK, registryName)), Blocks.OAK_PLANKS.defaultBlockState()), block, item);
                        }
                        else if(isBlockSetType(reflectedLocation)) {
                            CreateBlockReferences.createBlock(id, registryName -> Registration.getBlocksField(blockId.reflectedLocation().getForge(), prop.setId(ResourceKey.create(Registries.BLOCK, registryName)), blockSetType), block, item);
                        }
                        else {
                            if (isFence(reflectedLocation)) {
                                CreateBlockReferences.createBlock(id, registryName -> new FenceBlock(prop.setId(ResourceKey.create(Registries.BLOCK, registryName))), block, item);
                        ...
                        CreateBlockReferences.createBlock(id, registryName -> new Block(prop.setId(ResourceKey.create(Registries.BLOCK, registryName))), block, item);
                    }
                }
            }
        }
        ...
        for(ModType mod : type)
        {
            for(Map.Entry<String, SoundType> map : orderedMap.entrySet())
            {
                ...
                for(BlockId blockId : mcwBlockIdBase.blocks())
                {
                    ...
                    {
                        if(isBlockDefaultState("1.17.1", reflectedLocation)) {
                            CreateBlockReferences.createBlockStone(id, registryName -> Registration.getBlocksField(blockId.reflectedLocation().getForge(), prop.setId(ResourceKey.create(Registries.BLOCK, registryName)), Blocks.STONE.defaultBlockState()), block, item);
                        }
                        else {
                            if (isFence(reflectedLocation)) {
                                CreateBlockReferences.createBlockStone(id, registryName -> new FenceBlock(prop.setId(ResourceKey.create(Registries.BLOCK, registryName))), block, item);
                            } else if (isFenceGate(reflectedLocation)) {
                                CreateBlockReferences.createBlockStone(id, registryName -> new FenceGateBlock(woodType, prop.setId(ResourceKey.create(Registries.BLOCK, registryName))), block, item);
                            }  else {
                                CreateBlockReferences.createBlockStone(id, registryName -> Registration.getBlocksField(blockId.reflectedLocation().getForge(), prop.setId(ResourceKey.create(Registries.BLOCK, registryName))), block, item);
                            }
                        }
                    }
                    else {
                        CreateBlockReferences.createBlockStone(id, registryName -> new Block(prop.setId(ResourceKey.create(Registries.BLOCK, registryName))), block, item);
                    }
                }
            }
        }
