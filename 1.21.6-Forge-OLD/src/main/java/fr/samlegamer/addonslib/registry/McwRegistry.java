    ...
    public static void setRegistriesLeave(Map<String, SoundType> stringSoundTypeMap, DeferredRegister<Block> block, DeferredRegister<Item> item)
    {
        ...
        for(BlockId blockId : mcwBlockIdBase.blocks())
        {
            for(Map.Entry<String, SoundType> map : orderedMap.entrySet())
            {
                ...
                BlockBehaviour.Properties prop;
                boolean isModLoaded = modList.isLoaded(mcwBlockIdBase.modid());
                prop = McwProperties.getLeaveProperties().sound(soundType).setId(block.key(id));
        ...
        for(ModType mod : type)
        {
            for(Map.Entry<String, SoundType> map : orderedMap.entrySet())
            {
                ...
                for(BlockId blockId : mcwBlockIdBase.blocks())
                {
                    ...
                    BlockBehaviour.Properties prop;
                    boolean isModLoaded = modList.isLoaded(mcwBlockIdBase.modid());
                    prop = McwProperties.getStoneProperties(mod).sound(soundType).setId(block.key(id));
