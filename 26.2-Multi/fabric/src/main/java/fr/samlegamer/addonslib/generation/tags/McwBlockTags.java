...
public abstract class McwBlockTags extends FabricTagsProvider.BlockTagsProvider implements ITag
{
    public McwBlockTags(FabricPackOutput output, CompletableFuture<HolderLookup.Provider> registriesFuture) {
    ...
    public void buildToTagSystem(McwBlockIdBase mcwBlockIdBase, String modid, List<String> MAT)
    {
        ...
        for(Map.Entry<DoubleObject<String, String>, String> entry : tags.entrySet())
        {
            ...
            {
                this.tag(getTag(tagName)).add(block.builtInRegistryHolder().key());
            }
        }
    }
