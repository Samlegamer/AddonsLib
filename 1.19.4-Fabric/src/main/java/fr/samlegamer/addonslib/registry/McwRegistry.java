...
import fr.samlegamer.addonslib.RegistrationFabric;
...
import static fr.samlegamer.addonslib.registry.RegistryUtils.makeDefaultFromList;
                ...
                for(BlockId blockId : mcwBlockIdBase.blocks())
                {
                    ...
                    if(isModLoaded)
                    {
                        if(isBlockDefaultState("1.18.2Fabric", reflectedLocation)) {
                            RegistryEntryReferences.registryEntryWood(MODID, id, RegistrationFabric.getBlocksField(blockId.reflectedLocation().getFabric(), prop, Blocks.OAK_PLANKS.defaultBlockState()));
                        }
                        else if(isBlockSetType(reflectedLocation)) {
                            RegistryEntryReferences.registryEntryWood(MODID, id, RegistrationFabric.getBlocksField(blockId.reflectedLocation().getFabric(), prop, blockSetType));
                        }
                        else {
        ...
        for(ModType mod : type)
        {
            for(Map.Entry<String, SoundType> map : orderedMap.entrySet())
            {
                ...
                for(BlockId blockId : mcwBlockIdBase.blocks())
                {
                    ...
                    {
                        if(isBlockDefaultState("1.18.2Fabric", reflectedLocation)) {
                            RegistryEntryReferences.registryEntryStone(MODID, id,  RegistrationFabric.getBlocksField(blockId.reflectedLocation().getFabric(), prop, Blocks.STONE.defaultBlockState()));
                        }
                        else {
                            if (isFence(reflectedLocation)) {
                                RegistryEntryReferences.registryEntryStone(MODID, id,  new FenceBlock(prop));
                            } else if (isFenceGate(reflectedLocation)) {
                                RegistryEntryReferences.registryEntryStone(MODID, id,  new FenceGateBlock(prop, woodType));
                            }  else {
                                RegistryEntryReferences.registryEntryStone(MODID, id,  RegistrationFabric.getBlocksField(blockId.reflectedLocation().getFabric(), prop));
                            }
                        }
                    }
                    else {
                        RegistryEntryReferences.registryEntryStone(MODID, id,  new Block(prop));
                    }
                }
            }
        }
